  ...
  const processSingleGroup = async (groupId: string, isRetry: boolean = false): Promise<{ success: boolean, isRateLimit?: boolean }> => {
    ...
    while (!success && attempts < maxAttempts) {
      ...
      try {
        ...
        // INCREMENT USAGE BEFORE AI CALL TO ENSURE STRICT LIMIT
        try {
        ...
        if (isJsonError) {
          ...
          const cooldownTime = 5 * 1000; // 5 seconds cooldown for rate limits
          apiKeysRef.current = {
            ...apiKeysRef.current,
          ...
          globalCooldownUntilRef.current = Date.now() + 2000; // 2 second global pause
          ...
          backoffDelay = Math.min(backoffDelay * 2, 32000); // Exponential backoff up to 32s
        } else {
  ...
  const processFiles = async (files: File[]) => {
    ...
    const processedEpsFiles = await Promise.all(epsFiles.map(async (file) => {
        try {
            ...
            const res = await fetch('/api/convert-eps', { 
                method: 'POST', 
                body: formData,
                headers: { 'Authorization': `Bearer ${token}` }
            });
            ...
            return { file: file, previewUrl: '/' + data.preview };
        } catch (e) {
